package com.nik.customer.controller;

import com.nik.customer.client.ProductsClient;
import com.nik.customer.controller.payload.NewProductReviewPayload;
import com.nik.customer.entity.Product;
import com.nik.customer.service.FavoriteProductService;
import com.nik.customer.service.ProductReviewsService;
...
public class ProductController {
    ...
    private final FavoriteProductService productService;
    ...
    public Mono<String> getProductPage(@PathVariable("productId") int id, Model model) {
        ...
        return this.productReviewsService.findProductReviewsByProduct(id)
                ...
                .then(this.productService.findFavouriteProductByProduct(id)
    ...
    public Mono<String> addProductToFavourites(@ModelAttribute("product") Mono<Product> productMono) {
        return productMono
                .map(Product::id)
                .flatMap(productId -> this.productService.addProductToFavourites(productId)
    ...
    public Mono<String> removeProductFromFavourites(@ModelAttribute("product") Mono<Product> productMono) {
        return productMono
                .map(Product::id)
                .flatMap(productId -> this.productService.removeProductFromFavourites(productId)
                        .thenReturn("redirect:/customer/products/%d".formatted(productId)));
    }
    ...
    public String handelNoSuchElementException(NoSuchElementException exception, Model model){
